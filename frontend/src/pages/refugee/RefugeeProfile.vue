  ...
  <div class="space-y-6">
    ...
    <div class="card">
      ...
      <div class="flex items-center gap-4 sm:gap-6">
        <div class="relative">
          <div
            class="w-20 h-20 rounded-2xl overflow-hidden bg-primary/10 flex items-center justify-center border-2 border-primary/20"
          >
            <img
              v-if="form.profilePicturePreview || user?.profilePicture?.url"
              :src="form.profilePicturePreview || user.profilePicture.url"
    ...
    <div class="card space-y-5">
      ...
      <div>
        <label class="label"
          >Bio
          <span class="text-gray-400 font-normal">(Max 500 chars)</span></label
        >
        <textarea
          v-model="form.bio"
          rows="4"
          class="textarea-field"
          placeholder="Write a brief introduction about yourself, your background, and aspirations..."
          maxlength="500"
        ></textarea>
    ...
    <div class="card space-y-4">
      ...
      <div class="flex flex-wrap gap-2">
        <span
          ...
          :key="i"
          class="inline-flex items-center gap-1.5 bg-primary/10 text-primary px-3 py-1 rounded-full text-sm font-medium"
        >
          {{ skill }}
          <button
            @click="removeSkill(i)"
    ...
    <div class="card space-y-4">
      ...
      <button
        @click="
          form.languages.push({ language: '', proficiency: 'conversational' })
        "
        class="btn-ghost text-sm border border-dashed border-gray-300 inline-flex items-center gap-1.5"
      >
...
import { Camera, X, Trash2, Plus, Loader2 } from "lucide-vue-next";
...
onMounted(() => {
  ...
  if (u) {
    Object.assign(form, {
      ...
      refugeeStatus: u.refugeeStatus || "",
      bio: u.bio || "",
      skills: [...(u.skills || [])],
      languages: JSON.parse(JSON.stringify(u.languages || [])),
    });
  }
});
</script>
